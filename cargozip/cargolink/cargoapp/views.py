...
def dashboard(request):
    trips = Trip.objects.filter(Q(departure_time__gte=datetime.now()) | Q(driver__car_space__gte=F('trip_space'))).annotate(space_left=F('trip_space') - Sum('booking__cargo_size'))[0:4]
    bookings = Booking.objects.filter(passenger=request.user.pk).annotate(total_trips=Count('trip')).annotate(space_left=F('trip__trip_space') - Sum('cargo_size'))
...
def driverDashboard(request):
    ...
    if driver:
        ...
        trips = Trip.objects.filter(driver=None).annotate(space_left=F('trip_space') - Sum('booking__cargo_size'))
        mytrips = Trip.objects.filter(driver=request.user.pk).annotate(total_gains=Sum('booking__price')).annotate(total_bookings=Count('booking')).annotate(space_left=F('trip_space') - Sum('booking__cargo_size'))
...
@login_required(login_url='login')
def trips(request):
    trips = Trip.objects.filter(Q(departure_time__gte=datetime.now()) | Q(driver__car_space__gte=F('trip_space'))).annotate(space_left=F('trip_space') - Sum('booking__cargo_size'))

    context = {
        "trips":trips
    }

    trips = Trip.objects.filter(Q(departure_time__gte=datetime.now()) | Q(driver__car_space__gte=F('trip_space')) | Q(trip_space__lte=5000))
...
def search(request):
    ...
    if query:
        queryset = Trip.objects.filter(
            Q(departure_time__gte=datetime.now()) | Q(driver__car_space__gte=F('trip_space')) | Q(trip_space__lte=5000)|
...
def tripDetail(request, trip_id):
    ...
    total_gains = trip.booking_set.aggregate(total_gains=Sum('price'))['total_gains']
    total_bookings = trip.booking_set.aggregate(total_bookings=Count('id'))['total_bookings']
    space_left = trip.booking_set.aggregate(space_left=F('trip__trip_space') - Sum('cargo_size'))['space_left']
    bookings = Booking.objects.filter(trip = trip.pk)
...
def acceptTrip(request, trip_id):
    booking = Booking.objects.filter(trip__driver=request.user.pk)
    if not booking.exists():
        trip = Trip.objects.get(id=trip_id)
        trip.driver = request.user.driver
        current_space = Booking.objects.filter(trip=trip).aggregate(space_left=Sum('cargo_size'))['space_left']

        trip.trip_space = trip.driver.car_space
        trip.save()
        messages.success(request, 'Trip accepted succesfully')
        return redirect('driver_dashboard')
    else:
        messages.error(request, 'You are already on a trip')
        return redirect('driver_dashboard')
